...
VpcId=`aws ec2 create-vpc --cidr-block $2 --query 'Vpc.VpcId' --output text`
aws ec2 create-tags --resources $VpcId --tags Key=Name,Value=$6-Vpc
echo 'VPC created successfully!'
##############GET AVAILABILITY-ZONES FROM $1
zone1=`aws ec2 describe-availability-zones --region $1 --query 'AvailabilityZones[0].ZoneId' --output text`
zone2=`aws ec2 describe-availability-zones --region $1 --query 'AvailabilityZones[1].ZoneId' --output text`
zone3=`aws ec2 describe-availability-zones --region $1 --query 'AvailabilityZones[2].ZoneId' --output text`

##############CREATE SUBNET
subnet1=`aws ec2 create-subnet --availability-zone-id $zone1 --vpc-id $VpcId --cidr-block $3 --query 'Subnet.SubnetId' --output text`
aws ec2 create-tags --resources $subnet1 --tags Key=Name,Value=$6-Subnet1
echo 'first subnet created successfully!'
subnet2=`aws ec2 create-subnet --availability-zone-id $zone2 --vpc-id $VpcId --cidr-block $4 --query 'Subnet.SubnetId' --output text`
aws ec2 create-tags --resources $subnet2 --tags Key=Name,Value=$6-Subnet2
echo 'second subnet created successfully!'
subnet3=`aws ec2 create-subnet --availability-zone-id $zone3 --vpc-id $VpcId --cidr-block $5 --query 'Subnet.SubnetId' --output text`
aws ec2 create-tags --resources $subnet3 --tags Key=Name,Value=$6-Subnet3
...
aws ec2 create-tags --resources $gatewayId --tags Key=Name,Value=$6-Gateway
...
aws ec2 create-tags --resources $routeTableId --tags Key=Name,Value=$6-RouteTable
